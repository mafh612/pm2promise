...
describe('test pm2promise launchBus', () => {
  test('launchBus instance', async () => {
    expect(launchBus).toBeInstanceOf(Function)
  })

  test('launchBus call', async () => {
    const bus: SubEmitterSocket = new SubEmitterSocket()
    const fake_function: (
      cb: (err: Error, bus: SubEmitterSocket) => void
    ) => void = (cb: (err: Error, bus: SubEmitterSocket) => void): void => {
      cb(undefined, bus)
    }

    spyOn(pm2, 'launchBus').and.callFake(fake_function)

    await expect(launchBus()).resolves.toEqual(bus)
  })
  ...
  test('launchBus throw', async () => {
    ...
    await expect(launchBus()).rejects.toThrow('fake error')
  })
})
