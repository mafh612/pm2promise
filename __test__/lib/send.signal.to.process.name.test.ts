...
describe('test pm2promise sendSignalToProcessName', () => {
  test('describe instance', async () => {
    expect(sendSignalToProcessName).toBeInstanceOf(Function)
  })
  ...
  test('sendSignalToProcessName call', async () => {
    ...
    await expect(sendSignalToProcessName(1, 'process')).resolves.toEqual(
      'result'
    )
  })
  ...
  test('sendSignalToProcessName throw', async () => {
    ...
    await expect(sendSignalToProcessName(1, 'process')).rejects.toThrow(
      'fake error'
    )
  })
})
